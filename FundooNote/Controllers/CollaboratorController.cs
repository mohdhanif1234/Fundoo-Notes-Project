    ...
    public class CollaboratorController : ControllerBase
    {
        ...
        public IActionResult GetCollaboratorDetails(int noteId)
        {
            try
            {
                ...
                if (result != null)
                {
                    return this.Ok(new ResponseModel<List<CollaboratorModel>>() { Status = true, Message = "Collaborator details fetched successfully", Data = result });
                }
                else
                {
                    return this.BadRequest(new ResponseModel<string>() { Status = false, Message = "Collaborator details not found" });
                }
            }
            catch (Exception ex)
            {
